// Pure C (scalar) kernel implementations for gemma4.c.
//
// These are the baseline kernels from commit 7155eff (before AVX2 was added).
...
// It provides the same interface as kernels.c:
//   quantize, matmul_int8, attention_scores, weighted_value_sum, geglu
//
// The other kernels (embedding, rmsnorm, add_and_scale, apply_rope, softmax)
// live in transformer.c and are already portable.
//
// For standalone porting without transformer.c, see kernels_pure.h which
// declares the full set of 13 kernels.
