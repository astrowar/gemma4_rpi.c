...
ifeq ($(KERNELS),pure)
  KERNEL_SRC = kernels_pure_int8.c kernels_pure_int4.c
  CFLAGS = $(CFLAGS_BASE)
  ARCH = pure
else ifeq ($(KERNELS),avx2)
  KERNEL_SRC = kernels_avx_int8.c kernels_avx_int4.c
  CFLAGS = $(CFLAGS_BASE) -mavx2 -mfma -mf16c
  ARCH = avx2
else ifeq ($(KERNELS),neon)
  KERNEL_SRC = kernels_neon_int8.c kernels_neon_int4.c
...
test-neon:
	$(CC) $(CFLAGS_BASE) -mcpu=cortex-a72 -O2 test_neon_matmul.c kernels_neon_int8.c -o test_neon_matmul $(LDFLAGS)
	./test_neon_matmul
...
test-int4-pure:
	$(CC) $(CFLAGS_BASE) -O2 test_int4.c kernels_pure_int4.c -o test_int4_pure $(LDFLAGS)
	./test_int4_pure

test-int4-neon:
	$(CC) $(CFLAGS_BASE) -mcpu=cortex-a72 -O2 test_int4.c kernels_neon_int4.c -o test_int4_neon $(LDFLAGS)
	./test_int4_neon
